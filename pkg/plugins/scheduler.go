...
func (cs *CustomScheduler) PreFilter(ctx context.Context, state *framework.CycleState, pod *v1.Pod) (*framework.PreFilterResult, *framework.Status) {
	...
	// You can use “SelectorFromSet” to create the selector

	// 1. extract the label of the pod
	podLabels := pod.ObjectMeta.Labels
	podGroup := podLabels["podGroup"]
	minAvailable := podLabels["minAvailable"]
	intMinAvailable, _ := strconv.Atoi(minAvailable)
	...
	// 3. justify if the pod can be scheduleds
	if len(pods) < intMinAvailable {
		newStatus = framework.NewStatus(framework.Unschedulable, "Pods in group are less than minAvailable")
	}
...
func (cs *CustomScheduler) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	allocatable_memory := nodeInfo.Allocatable.Memory
	// 2. return the score based on the scheduler mode
	switch cs.scoreMode {
	case "Most":
		return allocatable_memory, framework.NewStatus(framework.Success, cs.scoreMode)
	case "Least":
		return -allocatable_memory, framework.NewStatus(framework.Success, cs.scoreMode)
	default:
